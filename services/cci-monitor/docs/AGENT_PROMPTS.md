...
1. **先做 Story 2.4 横截面相关性**(项目基石,整个 MVP 的最高优先级)
2. 再做 Story 2.1/2.2/2.3(其他三条信号,并行或顺序均可)
3. 然后 Story 2.5 CCI 合成(依赖 2.4)
4. 最后 Story 2.6 daily_service(集成所有)

**不要按照 Story 编号顺序开发!2.4 必须最先完成。**

---

### Story 2.4 第一阶段: 核心算法 ⭐⭐⭐ **两天专注投入**

```
Story 2.4 是整个 CCI Monitor 项目的基石,也是工时最长的单个 Story。
请你**务必**按以下分阶段开发,不要一次性实现所有功能。

本次对话只完成第一阶段: **核心算法 `compute_rho_bar_fast`**。

请:
1. 仔细阅读 docs/CCI_Monitor_Epic_Stories.md 中 Story 2.4 的完整说明
2. 仔细阅读 docs/CCI_Monitor_Technical_Spec.md 中"横截面相关性矢量化实现"章节
3. 重点理解:为什么不能用 pandas.corr() 循环?为什么要用 numpy 矢量化?

然后只实现这一个函数:
- compute_rho_bar_fast(returns_matrix, window, min_stocks, min_valid_ratio)

完成后请:
1. 写三个正确性单元测试:
   - test_rho_bar_independent_stocks (期望 ρ̄≈0)
   - test_rho_bar_synchronized_stocks (期望 ρ̄≈1)
   - test_rho_bar_partial_correlation (50/50 混合,期望 ρ̄≈0.5)
2. 写一个性能基准测试:
   - test_performance_300x250 (必须 < 3 秒)
3. 运行所有测试,告诉我结果

不要实现 compute_cross_correlation 或形态分类,这些留到第二阶段。
如果性能不达标或测试不通过,**停下来告诉我**,不要继续。
```

### Story 2.4 第二阶段: 完整结果与形态分类

```
Story 2.4 第一阶段已完成(compute_rho_bar_fast 通过所有测试)。
现在实现第二阶段: **完整的 compute_cross_correlation + 形态分类**。

请:
1. 实现 CrossCorrelationResult dataclass
2. 实现 compute_cross_correlation(returns_wide, window, min_stocks)
   - 调用第一阶段的 compute_rho_bar_fast 得到主序列
   - 分别计算涨日 rho_up 和跌日 rho_down
   - 计算 delta_rho (20日变化) 和 up_down_ratio
3. 实现 _classify_patterns 形态分类函数
   - A_rally: 齐涨型
   - B_crash: 齐跌型
   - C_crowding: 齐震型
   - normal: 正常
...
单元测试:
- 测试 CrossCorrelationResult 所有字段正确填充
- 测试形态分类逻辑(构造齐涨/齐跌/齐震三种场景数据)

完成后运行测试,告诉我三种形态是否都能正确识别。
```

### Story 2.4 第三阶段: 真实数据集成验证

```
Story 2.4 前两阶段已完成。现在用真实 A 股数据验证。

请:
1. 写一个集成测试(标记 @pytest.mark.integration):
   - 用 AkshareDataSource 拉取沪深300前50只成分股近1年数据
   - 运行 compute_cross_correlation
   - 验证:
     * ρ̄ 序列不全为 NaN
     * ρ̄ 值在 0.1-0.8 合理区间
     * 平均 ρ̄ 在 0.15-0.65 之间(真实市场的典型范围)

2. 用 matplotlib 画一张 ρ̄ 时间序列图
   - x 轴日期
   - y 轴 ρ̄
   - 标注 2024.09.24 政策反弹节点,看 ρ̄ 有没有明显上升
   - 保存为 data/artifacts/rho_bar_validation.png

3. 告诉我:
   - 真实数据的 ρ̄ 最小值/最大值/平均值是多少
   - 2024.09 前后 ρ̄ 的变化趋势是怎样的
   - 你观察到的任何有意思的现象

这一步通过后,Story 2.4 算完成。可以继续 Story 2.1/2.2/2.3。
```

---

### Story 2.1-2.3: 三条信号(在 2.4 完成后进行)

```
Story 2.4 横截面相关性已通过所有测试。现在并行实现其他三条信号。

基于 docs/CCI_Monitor_Epic_Stories.md 的 Story 2.1, 2.2, 2.3:
...
关键要求:
- 所有函数返回 pd.DataFrame,index 与输入一致
- 数据量不足时抛 InsufficientDataError
- 完整的 docstring

每个信号都写单元测试(参考 docs 中的示例):
- 常数输入的边界行为
- 已知分布的期望输出
- 数据不足时的异常

三个 Story 可以一起做,每完成一个运行测试确认。
```
...
2. 缺失参数使用中性值(参考文档)
3. 权重和必须为 1,否则抛 ConfigurationError
4. 实现 classify_alert_level 分类函数

测试:
- baseline 场景(market_rho=0.25, resonant/deep=0.78, delta=0.05, ratio=1.1):CCI 在 0.5-0.9
- critical 场景(market_rho=0.65, resonant/deep=1.33, delta=0.20, ratio=1.8):CCI > 1.3 且 alert_level=3
- 权重和不为 1 时抛 ConfigurationError
```

### Story 2.6 · Milestone 2 Checkpoint: 集成为 daily_service

```
现在把 Milestone 2 的所有组件串起来,实现完整的 daily_service。
...
1. 拉取沪深300前300只成分股
2. 批量拉取近120天数据
3. 计算横截面相关性(获取 market_rho, delta_rho, up_down_ratio)
4. 合成 CCI
5. 写入数据库(层级=1 全市场)
6. 返回 CCIResult

验证(⭐ Milestone 2 Checkpoint):
1. 运行 `python scripts/run_daily.py`
2. 检查控制台输出今日 CCI
3. 查询数据库确认 cci_records 表有新记录
4. 再运行一次,确认 UniqueConstraint 生效不会重复插入

完成 Checkpoint 即 Milestone 2 完成,可以进入 Milestone 3。
```
