...
from service import PywencaiService
...
logger = logging.getLogger("pywencai-source")

class Server:
    def __init__(self):
        self.server = None
        self.nacos_client = None
        self.service_name = os.getenv("SERVICE_NAME", "pywencai-source")
        self.service_host = os.getenv("SERVICE_HOST", "127.0.0.1")
        self.service_port = int(os.getenv("SERVICE_PORT", "50053"))
        ...
        try:
            ...
            self.nacos_client.add_naming_instance(
                ...
                metadata={
                    "version": "1.0.0", 
                    "type": "grpc", 
                    "capabilities": "SCREENING,RANKING,SECTOR"
                }
            )
