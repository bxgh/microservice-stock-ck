...
### 功能需求
- [x] 实现飙升榜数据源自动晋升（替代5分钟涨幅计算）✅ `PromotionMonitor`
- [ ] 实现 5 分钟换手率 > 1% 的自动晋升
- [x] 晋升股票临时进入 L1 池（30 分钟），不挤出原有股票 ✅ `DynamicPoolManager`
- [x] 支持手动添加紧急监控股票 ✅ API `/api/v1/stock-pool/manual-add`

### 性能需求
- [x] 异动检测延迟 < 10 秒 ✅ 飙升榜每5分钟扫描
- [x] 晋升股票立即参与下一轮采集 ✅ `get_current_pool()` 合并
- [x] 支持同时晋升多只股票（最多20只）✅ `max_dynamic_size=20`

### 测试需求
- [x] 单元测试覆盖晋升/降级逻辑 ✅ 11 tests passed
- [ ] 模拟异动场景测试（需开盘验证）
- [x] 验证时间窗口过期机制 ✅ `cleanup_expired()`
