...
- `security_type` (string, 可选): 证券类型 (stock/index/etf/fund/bond/warrant)
- `is_active` (boolean, 可选): 是否活跃 (数据维护状态)
- `is_listed` (boolean, 可选): 是否上市 (True: 在市, False: 退市)
...
**查询参数**:
- `exchange`, `security_type`, `is_active`, `is_listed` (同上)
...
**查询参数 (可选)**:
- `security_type`, `exchange`: **建议提供**。由于同一代码可能对应不同交易所或证券类型（如 000001 既是平安银行股票也是上证指数），使用复合主键 `(code, security_type, exchange)` 可确保精确定位唯一记录。
