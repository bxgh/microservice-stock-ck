...
系统目前采用 **Node 41 集中化高并发采集架构**，将全市场（约 5300+ 只股票）的分笔数据同步及补采任务统一收拢至核心服务器。

### 1.1 协同模型
- **mootdx-api**: 数据网关，维护规模为 150+ 的 TDX 连接池，作为全链路吞吐量基石。
- **gsd-worker**: 任务执行器，部署在 Node 41，负责全量同步、质量审计及自愈。
- **task-orchestrator**: 调度器，负责工作流编排（同步 -> 审计 -> 修复 -> 归档）。
...
## 3. 节点配置快照 (2026-02-01 已集中)

Server 41 作为唯一的核心生产节点，Server 58 和 111 降级为故障冗余。

| 节点 | 角色 | TDX_POOL_SIZE | 并发数 (Main/Retry) | 备注 |
|------|------|---------------|----------------------|------|
| **Server 41** | **核心采集节点** | 150 | 60 / 60 | 万兆宽带，核心生产环境 |
| **Server 58** | 故障冗余 (Standby) | 10 | 0 / 10 | 辅助/测试 |
| **Server 111** | 故障冗余 (Standby) | 10 | 0 / 10 | 辅助/测试 |
...
### 7.4 场景四：定向个股紧急修复 (On-demand Repair)
- **方式**: 手动触发
- **运行逻辑**: 
    - 使用 `repair` 模式，必须明确指定 `--date` 和 `--stock-codes`。
    - **技术行为**：系统会先物理清空该股在 ClickHouse 中的当日脏数据，然后重新执行 V3 矩阵重叠抓取并入库。
- **手动触发命令**:
  ```bash
  docker run --rm --network host --env-file services/gsd-worker/.env \
    ...
    --entrypoint python gsd-worker:latest -m jobs.sync_tick --date 20260130 --mode repair --stock-codes 600519
  ```
- **目标**: 针对审计发现的个别异常股进行“手术级”精准修复。
