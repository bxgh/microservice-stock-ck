...
# 从配置加载核心指数
config = get_config()
CORE_INDICES = [f"{c}.SH" if c.startswith('0') or c.startswith('6') else f"{c}.SZ" for c in config.get("indices", [])]
# 北京指数特殊处理 (如果未来需要)
CORE_INDICES = [c for c in CORE_INDICES if not c.endswith('.BJ')] + [f"{c}.BJ" for c in config.get("indices", []) if c.startswith('8')]
# 修正：由于配置文件只给代码，我们需要根据代码自动补全后缀，或者在采集逻辑中统一 normalize 后对比
