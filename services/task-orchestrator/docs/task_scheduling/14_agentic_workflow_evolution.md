...
## 5. 执行机制 (Execution Mechanics): "Trust the Database"

这是 4.0 架构最重要的设计原则。

### 5.1 为什么不直接读 YAML 文件？
*   **分布式一致性**: Worker 分布在不同节点，无法保证每个节点上的文件都是最新的。数据库是唯一的真理来源 (Single Source of Truth)。
*   **版本隔离**: 当您在一个长流程运行中途修改了 YAML，正在运行的实例不应受影响。数据库快照机制解决了这个问题。
*   **热加载**: 修改流程无需重启服务。

### 5.2 流程生命周期
1.  **定义 (Define)**: 在 `config/workflows/*.yml` 编写逻辑。
2.  **同步 (Sync)**: 运行 `sync_workflows.py`。
    *   读取 YAML -> 解析为 JSON -> Upsert 到 `workflow_definitions` 表。
3.  **触发 (Trigger)**: 定时器/API 创建一个新的 `workflow_run`。
    *   **快照**: 系统会将当前的 definition 深拷贝一份，随 run 实例一起锁定。
4.  **执行 (Execute)**: `FlowController` 仅从 DB 读取任务状态并推进 Step。

> **⚠️ 注意**: 修改 YAML 后，必须手动运行 Sync 脚本，否则变更不会生效。
