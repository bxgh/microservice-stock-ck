...
> **问题**: mootdx 的分笔数据 API 需要指定 `start` 和 `offset` 参数，但 09:25 集合竞价数据位置不固定，且 API 有单次 2000 条（建议 800 条）的返回限制。

**解决方案**: 使用**顺序批次回溯策略 (Sequential Batch Fetching)**，从 15:00 开始向前按固定批次采集，直到覆盖目标时间点。

**算法逻辑**:
1. 从 `start=0` (收盘 15:00) 开始，每批采集 `batch_size=800` 条数据。
2. 逐批检查数据中的最早时间戳（`min(time)`）。
3. 如果最早时间戳仍晚于 `09:25`，则 `start += len(data)` 继续向前回溯。
4. 一旦发现 `< 09:25` 的数据或无更多数据，停止回溯。
5. 合并并过滤该交易日的所有分笔记录。

**优势**:
- **100% 覆盖**: 只要源端有数据，该策略能保证采集到全天完整序列。
- **自适应性**: 自动适应不同活跃度股票的数据量，无需预设偏移位。
- **稳定性**: 通过固定批次和重试机制，规避 API 超时风险。
