...
#### 验收标准
- [ ] 成功生成全市场 5000+ 股票的 240 分钟连续序列。
- [ ] 涨跌停股票的成交量分布与 K 线数据完全匹配。
- [ ] 自动剔除全天停牌或有效交易时间少于 30 分钟的僵尸股。
...
| **向量 B** | 盘口失衡度 (OBI) | 5档报单量对比 $w_i \times (B_i - S_i) / (B_i + S_i)$ |
| **向量 C** | 分时累积收益率 | $Log(P_t / P_{open})$ |

#### 验收标准
- [ ] 生成全天 240 维度的 Numpy 矩阵。
- [ ] OBI 计算包含线性权重衰减（近端权重高）。
- [ ] 特征计算延迟：全市场 5000 只股票计算总时长 < 5 分钟（需多进程优化）。
...
##### 2. 核心指标计算
- **LOR (大单占比)**：`(大单金额 + 超大单金额) / 总成交额`
- **NLB (大单净买入)**：`大单买入额 - 大单卖出额`
- **RID (机构/散户背离度)**：
  - +2：机构买 & 散户卖
  - -2：机构卖 & 散户买
  - 0：方向一致或无明确趋势
...
#### 验收标准
- [ ] 成功识别"机构合力"股票池（LOR > 0.5）。
- [ ] 计算结果存入 Redis，支持聚类分析调用。
- [ ] 性能：单股 3 秒分笔数据处理耗时 < 50ms。
...
#### 验收标准
- [ ] 实时/盘后计算 VPIN 并在 > 0.6 时记录警告。
- [ ] λ (Lambda) 能够识别出盘口变薄的敏感阶段。
