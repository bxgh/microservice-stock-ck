...
class DataQualityMonitor:
    ...
    def verify_tick_topology(self, ticks_df: pd.DataFrame) -> Tuple[str, str]:
        ...
        if not is_sorted:
            # Check deviation
            unsorted_count = (df['tick_time'].diff() < pd.Timedelta(0)).sum() if pd.api.types.is_timedelta64_dtype(df['tick_time']) else 0
    ...
    async def is_qualified(self, stock_code: str, trade_date: str, 
                         ...
                         snapshot_df: Optional[pd.DataFrame] = None) -> Tuple[bool, str]:
        """
        综合判定是否合格
        """
