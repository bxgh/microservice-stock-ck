#!/usr/bin/env python3
"""
Akshare Proxy API Service (Ultimate Robust - Json Fix)
为内网环境提供 Akshare 数据访问接口
支持 EPIC-002 (Quant Strategy) 和 EPIC-005 (Backtesting) 的所有需求
Version: 2.4.0 (Fix JSON Date Serialization)
"""
...
def safe_json_response(df: pd.DataFrame) -> Response:
    ...
    # 将 NaN 替换为 None (在转 JSON 前先处理)
    # 也可以直接利用 pandas 的 to_json 处理 null
    
    # 使用 pandas 的 to_json 直接序列化，支持 iso 日期格式
    try:
...
@app.get("/")
def root():
    return {"status": "running", "version": "2.4.0", "akshare_version": ak.__version__}
...
def get_stock_info(symbol: str):
    ...
    try:
        ...
        # Info dictionary usually simple types
        import json
