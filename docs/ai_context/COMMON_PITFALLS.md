...
> **2026-01-06 惨痛教训**: 手动验证通过 ≠ 调度器运行通过

| 问题 | 场景 | 根因 | 解决方案 |
|------|------|------|----------|
| **配置不一致** | 手动 `docker run` 成功，调度器失败 | `tasks.yml` 不会自动继承 `global` (当前代码缺陷) | **必须**在 `tasks.yml` 中显式传递环境变量，或修复 `TaskLoader` |
| **网络隔离** | 容器内连不上宿主机隧道 | 未设置 `network_mode: host` | 显式指定 `network_mode: host` |
| **Silent Failure** | 容器启动即退，调度器认为成功 | `remove=True` 且只检查启动成功 | 检查任务内部逻辑日志，使用 `attach` 或保留容器调试 |
| **虚假验证** | 仅依赖 `docker run` 测试 | 调度器启动路径完全不同 | **必须**通过调度器 Trigger 接口验证最终效果 |

**关键规则**:
1.  **调度器不是 Docker Compose**: 它不会自动加载 `.env`，也不会自动继承任何全局变量（除非代码显式实现了）。
2.  **网络模式陷阱**: Python Docker SDK 默认使用 `bridge` 网络。如果你的服务依赖宿主机的 `127.0.0.1` 隧道（如 MySQL/ClickHouse 隧道），**必须**显式传入 `network_mode='host'`。
3.  **验证标准**: 不要只信 `docker run`。必须强制触发一次调度器任务，并在数据库/日志中看到真实执行记录才算过关。
