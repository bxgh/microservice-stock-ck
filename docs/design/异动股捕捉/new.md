...
def identify_extreme_market(trade_date: date) -> dict:
    ...
    # TBD-1: 阈值 80,初版硬编码,落地后回测调
    is_broad_up = (regime == 'broad_up' and limit_up_count > 80)
    is_broad_down = (regime == 'broad_down' and limit_down_count > 80)
...
| ID | 内容 | 影响 | 默认处理 |
|---|---|---|---|
| **TBD-1** | 极端市况涨停数阈值(初版 80) | E4-S1 触发条件 | 第一版硬编码 80,3 个月后用回测数据调优 |
| **TBD-2** | 历史回填范围(建议近 250 个交易日) | E2-S2 工程量 | 默认近 250 个交易日,实施侧确认是否调整 |
| **TBD-3** | C3 资金驱动中"北向资金异常"具体阈值 | E3-S1 分类逻辑 | 第一版硬编码:单日北向净买金额 > 50 亿元(全市场)+ 个股持仓变动占比 TBD |
| **TBD-4** | C4 事件驱动是否需要进一步分 pos/neg/neutral | F3 升级时再决定 | 第一版仅用 has_event_today 全集,无方向区分 |
| **TBD-5** | D 视图前端展示形态(连板梯队是表格还是柱状图) | 前端实现细节 | 后端只产数据,前端形态由小程序决定 |
| **TBD-6** | 评估系统是否需要 ClickHouse 专用表(列存优化)| E2 性能 | 第一版仅 MySQL,若回测 > 30 分钟超时再考虑 CK 专用表 |
| **TBD-7** | F1 → F3 升级触发条件具体阈值(8pp 是经验值) | 长期路线图 | 暂记 8pp,跑满 60 个交易日数据后实测 |
